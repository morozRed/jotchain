...
export default function GoogleSignInButton({ children = "Continue with Google" }: GoogleSignInButtonProps) {
  const { props } = usePage<PageProps>()
  const csrfToken = props.csrf_token ?? document.querySelector('meta[name="csrf-token"]')?.getAttribute('content') ?? ''

  return (
    <form method="post" action="/auth/google_oauth2" style={{ width: "100%" }}>
